    ...
    >
      ...
      {isUploading ? (
        <div className="group grid w-full place-items-center gap-1 sm:px-10">
          <UploadCloud
            className="size-10 animate-pulse text-slate-700 dark:text-slate-400"
            aria-hidden="true"
          />
        ...
        previewType === "image" ? (
          <div className={cn(files.length > 1 ? "grid gap-2" : "")}>
            <div className="group relative aspect-square size-full max-h-[420px] overflow-hidden rounded-lg">
              {isDragActive ? (
        ...
        <div className="grid place-items-center gap-1 sm:px-10">
          <UploadCloud
            className="size-10 text-slate-700 dark:text-slate-400"
            aria-hidden="true"
          />
...
const DragActive = ({ isDragActive }: { isDragActive: boolean }) => {
  return (
    <div className="grid place-items-center gap-2 text-slate-700 dark:text-slate-400 sm:px-10">
      <UploadCloud
        className={cn("size-10", isDragActive ? "animate-bounce" : "")}
        aria-hidden="true"
      />
      <p className="text-base font-medium sm:text-lg">Drop files here</p>
    </div>
  )
}
